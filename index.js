...
http.createServer(function(req, res) {                               // Then, you want to create a server
  ...
  } catch(err) {                                                        // if not, respond with the error
  ...
  if(stats.isFile()) {                                                                 // if there is a file...
    let mimeTypes = mimeTypes[path.extname(fileName).split(".").reverse()[0]];         // find out what type of file it is
